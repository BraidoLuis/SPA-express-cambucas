...
import {
  ...
  getAvailableSlots,
  type AdminAppointmentClient,
  type AdminAppointmentOptions,
  type AvailableSlot,
...
export function AdminAppointmentForm({
  ...
  open: boolean;
  initialClient?: AdminAppointmentClient | null;
  onClose: () => void;
  ...
  const client = options.clients.find((x) => x.id === clientId);
  const slot = slots.find((x) => x.start === slotStart);

  const normalizedSearch = search.trim().toLocaleLowerCase("pt-BR");
  const visibleClients = options.clients
    .filter(
      (item) =>
        !normalizedSearch ||
  ...
  function review(event: FormEvent) {
    ...
    if (!client || !service || !professional || !slot) {
  ...
  async function confirm() {
    ...
    try {
      const id = await createAdminAppointment({
        clientId: client.id,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="eyebrow">AGENDAMENTO ADMINISTRATIVO</span>
        <h2 id="admin-appointment-title">Novo agendamento</h2>
        <p>Reserve um horário real em nome de uma cliente. Pagamento no local.</p>

        {loading ? (
          <div className="admin-data-message">Carregando opções...</div>
        ) : success ? (
        ...
        ) : reviewing && client && service && professional && slot ? (
          <div className="admin-appointment-review">
            ...
            <dl>
              ...
              <div>
                <dt>Duração e término</dt>
                <dd>
                  {professional.duration} minutos ·{" "}
                  {new Intl.DateTimeFormat("pt-BR", {
                    timeStyle: "short",
                    timeZone: "America/Sao_Paulo",
          ...
          <form onSubmit={review} className="admin-appointment-form">
            <fieldset>
              <legend>1. Cliente</legend>
              {client ? (
                <div className="selected-admin-client">
                  <div>
                    <b>{client.name}</b>
                    <span>
                      {client.email}
                      {client.phone ? ` · ${formatBrazilianPhone(client.phone)}` : ""}
                    </span>
                  </div>
                  <button
                    type="button"
                ...
                <>
                  ...
                  <div className="admin-client-options">
                    {visibleClients.map((item) => (
                      <button type="button" key={item.id} onClick={() => setClientId(item.id)}>
                        <b>{item.name}</b>
                        <span>{item.email}</span>
                      </button>
                    ))}
                    {!visibleClients.length && <p>Nenhuma cliente ativa encontrada.</p>}
                  </div>
                </>
              )}
            </fieldset>
            ...
            <div className="admin-flow-grid">
              <label>
                2. Serviço
                <select value={serviceId} onChange={(e) => chooseService(e.target.value)}>
                ...
                <div className="admin-slot-options">
                  {slotsLoading ? (
                    ...
                    slots.map((item) => (
                      <button
                        type="button"
                        className={slotStart === item.start ? "active" : ""}
                  ...
                  {!slotsLoading && professional && date && !slots.length && (
            ...
            {professional && (
              <p className="admin-effective-values">
                Duração: <b>{professional.duration} minutos</b> · Valor no local:{" "}
                <b>
                  {professional.price.toLocaleString("pt-BR", {
                    style: "currency",
                    currency: "BRL",
            ...
            <footer>
              ...
              <button className="primary" disabled={!client || !service || !professional || !slot}>
                Revisar agendamento
              </button>
            </footer>
          </form>
