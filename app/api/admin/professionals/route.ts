...
export async function POST(request: Request) {
  ...
  if (profile.error || profile.data.role !== "admin" || !profile.data.active) {
    return NextResponse.json(
      { error: "Apenas administradores ativos podem convidar profissionais." },
      { status: 403 }
    );
  }
  ...
  const specialty = body.specialty?.trim().replace(/\s+/g, " ") || "";

  if (name.length < 3 || !emailPattern.test(email) || specialty.length < 2) {
    return NextResponse.json(
      { error: "Revise nome, e-mail e especialidade." },
      { status: 400 }
    );
  }
